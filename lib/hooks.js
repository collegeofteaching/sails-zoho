 ...
 * @param {Function} cb - function that is called when this hook finishes
 */
function createModulePointer(req, method, config, conn, cb){
  if(_.isObject(conn.options) && conn.options.hasOwnProperty('id')){
    config.module = conn.collection
    config.entity = conn.options.id
    console.log(conn.options.id)
  } else {
    config.module = conn.collection
  }
  cb();
...
module.exports = {
  before: [removeWhereFromQuery, createModulePointer],
